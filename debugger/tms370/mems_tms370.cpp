...
uint32_t MemsTms370::disassemble(uint32_t addr, uint8_t numInsn) const {
#ifdef WITH_DISASSEMBLER
    addr = Mems::disassemble(addr, numInsn);
    // Oscilator falut must be reset.
    _pins->resetCpu();
#endif
    return addr;
}

uint32_t MemsTms370::assemble(uint32_t addr, const char *line) const {
#ifdef WITH_ASSEMBLER
    addr = Mems::assemble(addr, line);
    // Oscilator falut must be reset.
    _pins->resetCpu();
#endif
    return addr;
