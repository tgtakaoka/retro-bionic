...
constexpr auto clkin_hi_ns = 200;    // 100
constexpr auto clkin_lo_ns = 200;    // 100
constexpr auto clkin_lo_dir = 120;   // 100
constexpr auto clkin_hi_addr = 100;  // 100
...
void PinsTms370C250::idle() {
    auto s = prepareCycle();
    completeCycle(s->inject(0x00));  // JMP $
    completeCycle(prepareCycle()->inject(0xFE));
    Signals::discard(s);
}
