...
constexpr uint16_t NMI_VECTOR = 0x0066;

constexpr uint8_t OPC_HALT = 0x76;
constexpr uint16_t OPC_RETN = 0xED45;  // ED,45 as one word fetch
...
Signals *PinsZ280::completeCycle(Signals *s) {
    ...
    if (s->read()) {
        if (s->memReq()) {
            if (s->readMemory()) {
                if (s->wordAccess()) {
                    s->data = swapBytes(_mems->read(s->addr));
                } else {
    ...
    if (s->write()) {
        if (s->memReq()) {
            if (s->writeMemory()) {
                if (s->wordAccess()) {
                    _mems->write(s->addr, swapBytes(s->data));
                } else {
...
uint16_t PinsZ280::suspend() {
    ...
    while (true) {
        auto s = prepareCycle();
        if (s->memReq() && s->read() && s->addr == NMI_VECTOR &&
                s->prev()->memReq() && s->prev()->write()) {
            negate_nmi();
            const auto pushedPc = s->prev()->data;
            completeCycle(s->inject(OPC_RETN));
...
bool PinsZ280::rawStep() {
    ...
    if (_mems->read_byte(pc) == OPC_HALT)
        return false;
    _regs->setIp(suspend());
    return true;
}
