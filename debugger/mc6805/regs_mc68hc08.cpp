...
void RegsMc68HC08::reset() {
    ...
    static constexpr uint8_t BSET_IRV[] = {
            0x1C, 0x3B,  // BSET 6,$3B; 1:2(:R:W)
    };
    _pins->injectReads(BSET_IRV, sizeof(BSET_IRV), 0);
    load_hx(0x1000);
    const uint8_t TXS = 0x94;
    _pins->injectReads(&TXS, sizeof(TXS), 0); // 1
}

void RegsMc68HC08::save() {
    RegsMc6805::save();
    const uint8_t PSHH = 0x8B;
    _pins->injectReads(&PSHH, sizeof(PSHH), 0); // 1:n:W
    _pins->captureWrites(&_h, sizeof(_h));
}

void RegsMc68HC08::load_hx(uint16_t hx) const {
    const uint8_t LDHX[] = {0x45, hi(hx), lo(hx)};  // LDHX #hx; 1:2:3
    _pins->injectReads(LDHX, sizeof(LDHX), 0);
}

void RegsMc68HC08::restore() {
    // Restore SP
    load_hx(_sp - 4 + 1);  // TXS: SP <- HX-1
    const uint8_t TXS = 0x94;
    _pins->injectReads(&TXS, sizeof(TXS), 0);
    // Restore H
    load_hx(uint16(_h, _x));
    const uint8_t RTI[] = {
        0x80, 0, _cc, _a, _x, hi(_pc), lo(_pc),
    };
    _pins->injectReads(RTI, sizeof(RTI), 0);
}
