...
constexpr auto ckin_lo_laddr = 20;    // 50
constexpr auto ckin_hi_maddr = 20;    // 50
constexpr auto ckin_lo_haddr = 20;    // 50
constexpr auto ckin_phi1_read = 0;    // 50
constexpr auto ckin_lo_inject = 20;   // 50
constexpr auto ckin_phi1_write = 0;   // 50
constexpr auto ckin_lo_get = 5;       // 50
constexpr auto ckin_hi_capture = 20;  // 50
constexpr auto ckin_lo_next = 0;      // 50
constexpr auto ckin_hi_input = 2;     // 50
...
tms9900::Signals *PinsTms9981::completeCycle(tms9900::Signals *_s) const {
    ...
    if (ready_asserted() && s->memory()) {
        if (s->read()) {
            delayNanoseconds(ckin_phi1_read);
            // phi2
            ckin_lo();
            if (s->readMemory()) {
            ...
            delayNanoseconds(ckin_phi1_write);
            // phi2
            ckin_lo();
