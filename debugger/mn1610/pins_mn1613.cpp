...
Signals *PinsMn1613::prepareCycle(bool checkHalt) const {
    auto s = Signals::put();
    while (true) {
        if (s->getControl())
            break;
        x2_cycle();
        if (checkHalt && isHalted())
            return s;
    }
    while (true) {
        if (signal_adsd() != LOW) {
...
void PinsMn1613::injectPsw(const uint16_t *data, uint_fast8_t len) {
    assert_hlt();
    for (uint_fast8_t i = 0; i < len; ++i) {
        const auto s = prepareCycle(false)->inject(data[i]);
        completeCycle(s);
        DEBUG(cli.print("@@ psw: "));
        DEBUG(s->print());
    }
