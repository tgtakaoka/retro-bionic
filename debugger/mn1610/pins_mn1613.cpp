...
constexpr auto x2_hi_ns = 20;   // 37.5 ns
constexpr auto x2_lo_ns = 20;   // 37.5 ns
constexpr auto x2_hi_pre = 20;  // 37.5 ns
constexpr auto x2_lo_pre = 20;  // 37.5 ns
...
auto signal_adsd() {
    return digitalReadFast(PIN_ADSD);
}

auto signal_dtsd() {
    return digitalReadFast(PIN_DTSD);
}
...
Signals *PinsMn1613::prepareReset() const {
    auto s = Signals::put();
    while (true) {
        x2_hi();
...
Signals *PinsMn1613::prepareCycle() const {
    auto s = Signals::put();
    while (true) {
        x2_hi();
        delayNanoseconds(x2_hi_pre);
...
Signals *PinsMn1613::rawPrepareCycle(Signals *s) const {
    while (true) {
        assert_debug();
        if (signal_adsd() != LOW) {
            s->getAddress();
...
Signals *PinsMn1613::completeCycle(Signals *s) const {
    ...
    if (s->read()) {
        if (s->readMemory()) {
            s->data = s->memory() ? _mems->read(s->addr) : _devs->read(s->addr);
        } else {
            delayNanoseconds(x2_lo_ns);
        }
        x2_lo();
...
void PinsMn1613::injectReset(const uint16_t *data, uint_fast8_t len) {
    ...
    for (uint_fast8_t i = 0; i < len; ++i) {
        const auto s = prepareReset()->inject(data[i]);
        completeCycle(s);
        DEBUG(cli.print("@@ psw: "));
        DEBUG(s->print());
    }
...
void PinsMn1613::halt() const {
    assert_hlt();
    while (true) {
        const auto s = prepareCycle()->inject(InstMn1613::NOP)->capture();
        if (isHalted()) {
            Signals::discard(s);
            return;
        }
...
void PinsMn1613::injectInst(const uint16_t *inst, uint_fast8_t len) {
    unhalt();
    for (uint_fast8_t i = 0; i < len; ++i) {
        const auto s = prepareCycle()->inject(inst[i]);
        completeCycle(s);
...
void PinsMn1613::captureInst(uint16_t inst, uint16_t *buf) {
    unhalt();
    auto s = prepareCycle()->inject(inst);
    completeCycle(s);
    DEBUG(cli.print("@@  inject: "));
    DEBUG(s->print());
    while (true) {
        s = prepareCycle()->inject(InstMn1613::NOP)->capture();
        completeCycle(s);
        if (s->write()) {
            *buf = s->data;
